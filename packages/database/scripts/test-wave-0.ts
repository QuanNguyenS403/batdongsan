...
import { OtpService } from '../../../apps/api/src/modules/auth/otp.service';
import { EmailService } from '../../../apps/api/src/modules/email/email.service';

const prisma = new PrismaClient();
const jwtService = new JwtService();
const otpService = new OtpService();
const authService = new AuthService(prisma as any, otpService, jwtService);
...
async function runWave0Verification() {
  ...
  // ---------------------------------------------------------------------------
  // TEST 1: P0-01 — Đảm bảo credential cũ không thể login hoặc tự động nâng quyền
  // ---------------------------------------------------------------------------
  ...
  try {
    ...
    backdoorFailed = err.status === 401 || err.message?.includes('không đúng');
  }
  ...
  assert(userCheckRole?.role === 'user', 'P0-01.4: Role của user thường vẫn giữ nguyên "user", không bị leo thang đặc quyền');
  ...
  try {
    ...
    blockedLoginFails = err.status === 401 && err.message?.includes('khóa');
  }
  assert(blockedLoginFails, 'P0-01.7: Tài khoản bị khóa (isBlocked=true) bị từ chối đăng nhập');

  // ---------------------------------------------------------------------------
  // TEST 2: Cơ chế Bootstrap Admin An toàn qua Secret ngoài repo
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // TEST 3: Safety Net Môi Trường Staging (Không gửi SMS / Email thật)
  // ---------------------------------------------------------------------------
  ...
  assert(stagingEmailService.isMock === true, 'Safety Net 3.1: EmailService tự động kích hoạt isMock=true trên môi trường Staging');
